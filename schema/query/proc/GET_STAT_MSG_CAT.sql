...
-- 프로시저 REDSPUSH.GET_STAT_MSG_CAT 구조 내보내기
DELIMITER //
CREATE DEFINER=`redspush`@`%` PROCEDURE `GET_STAT_MSG_CAT`(
    ...
    COMMENT '[oslee] 일별/주별/월별/년도별로 메세지 유형 통계 조회'
BEGIN
    ...
    IF  P_PROC_ID != 'STAT_CAT_WEEK' THEN
      ...
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.MSG_CAT_A), 0) AS '광고',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.MSG_CAT_B), 0) AS '업무',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.MSG_CAT_C), 0) AS '일정',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.MSG_CAT_E), 0) AS '기타' \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "FROM (     \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT A.DATE  AS DATE, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         (A.MSG_CAT_A +A.MSG_CAT_B+MSG_CAT_C+MSG_CAT_E) AS TOT,  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         A.MSG_CAT_A, A.MSG_CAT_B, A.MSG_CAT_C, A.MSG_CAT_E \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "  FROM(   \n");
      IF  P_PROC_ID = 'STAT_CAT_YEAR' THEN
        SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT DATE_FORMAT(TRNS_REQ_TS,'%Y') AS DATE,  \n");
      ELSEIF  P_PROC_ID = 'STAT_CAT_MONTH' THEN
        SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT DATE_FORMAT(TRNS_REQ_TS,'%Y-%m') AS DATE,  \n");
      ELSEIF  P_PROC_ID = 'STAT_CAT_DAY' THEN
        SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT DATE_FORMAT(TRNS_REQ_TS,'%Y-%m-%d') AS DATE,  \n");
      END IF;
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(MSG_CAT, '') = 'A' THEN 1 ELSE 0 END) AS MSG_CAT_A, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(MSG_CAT, '') = 'B' THEN 1 ELSE 0 END) AS MSG_CAT_B, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(MSG_CAT, '') = 'C' THEN 1 ELSE 0 END) AS MSG_CAT_C, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(MSG_CAT, '') = 'E' THEN 1 ELSE 0 END) AS MSG_CAT_E \n");
