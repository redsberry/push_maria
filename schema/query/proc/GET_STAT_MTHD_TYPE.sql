...
-- 프로시저 REDSPUSH.GET_STAT_MTHD_TYPE 구조 내보내기
DELIMITER //
CREATE DEFINER=`redspush`@`%` PROCEDURE `GET_STAT_MTHD_TYPE`(
    ...
    COMMENT '[oslee] 일별/주별/월별/년도별로 전송방법 통계 조회'
BEGIN
    ...
    IF  P_PROC_ID != 'STAT_TYPE_WEEK' THEN
      ...
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TRNS_MTHD_TYPE_D), 0) AS '즉시발송',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TRNS_MTHD_TYPE_P), 0) AS '반복발송',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TRNS_MTHD_TYPE_R), 0) AS '예약발송'  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "FROM (     \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT A.DATE  AS DATE, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         (A.TRNS_MTHD_TYPE_D +A.TRNS_MTHD_TYPE_P+TRNS_MTHD_TYPE_R) AS TOT,  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         A.TRNS_MTHD_TYPE_D, A.TRNS_MTHD_TYPE_P, A.TRNS_MTHD_TYPE_R \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "  FROM(   \n");
      IF  P_PROC_ID = 'STAT_TYPE_YEAR' THEN
        SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT DATE_FORMAT(TRNS_REQ_TS,'%Y') AS DATE,  \n");
      ELSEIF  P_PROC_ID = 'STAT_TYPE_MONTH' THEN
        SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT DATE_FORMAT(TRNS_REQ_TS,'%Y-%m') AS DATE,  \n");
      ELSEIF  P_PROC_ID = 'STAT_TYPE_DAY' THEN
        SET SQLSTRING = CONCAT(SQLSTRING, "  SELECT DATE_FORMAT(TRNS_REQ_TS,'%Y-%m-%d') AS DATE,  \n");
      END IF;
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(TRNS_MTHD_TYPE, '') = 'D' THEN 1 ELSE 0 END) AS TRNS_MTHD_TYPE_D, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(TRNS_MTHD_TYPE, '') = 'P' THEN 1 ELSE 0 END) AS TRNS_MTHD_TYPE_P, \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "         SUM(CASE WHEN IFNULL(TRNS_MTHD_TYPE, '') = 'R' THEN 1 ELSE 0 END) AS TRNS_MTHD_TYPE_R \n");
...
--       SET SQLSTRING = CONCAT(SQLSTRING, "    AND TRNS_STE = 'S'   \n");
    ...
    ELSE
      -- 주별통계
      SET SQLSTRING = CONCAT(SQLSTRING, "SELECT (CASE GB WHEN 'B' THEN '계' ELSE C.START END) '시작일자', \n"); 
      SET SQLSTRING = CONCAT(SQLSTRING, "        (CASE GB WHEN 'B' THEN '계' ELSE C.END END) '종료일자', \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TOT), 0) AS '계',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TRNS_MTHD_TYPE_D), 0) AS '즉시발송',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TRNS_MTHD_TYPE_P), 0) AS '반복발송',  \n");
      SET SQLSTRING = CONCAT(SQLSTRING, "        COALESCE(SUM(C.TRNS_MTHD_TYPE_R), 0) AS '예약발송'  \n");
